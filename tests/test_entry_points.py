...
`deskwright` (deskwright/cli.py) and the checkout's `./mcp_server.py` are
the same server by two names, and both go through `deskwright/session.py`. What they
...
self-test against `default` while `DESKWRIGHT_SESSION` still said `alpha`.
...
from deskwright import session as sess
...
def headless(monkeypatch):
    ...
    monkeypatch.delenv("DESKWRIGHT_HEADLESS_LAZY", raising=False)
    monkeypatch.delenv("DESKWRIGHT_SESSION", raising=False)
...
def test_unset_session_is_a_no_op(headless, monkeypatch):
    sess.resolve_session(as_server=True, argv=["deskwright"])
    assert headless["ensure"] == [] and headless["status"] == []
    assert "DESKWRIGHT_SESSION" not in __import__("os").environ


def test_primary_is_a_no_op(headless, monkeypatch):
    monkeypatch.setenv("DESKWRIGHT_SESSION", "primary")
    sess.resolve_session(as_server=True, argv=["deskwright"])
    assert headless["ensure"] == [] and headless["status"] == []
...
def test_flag_beats_environment(headless, monkeypatch):
    ...
    import os
    assert os.environ["DESKWRIGHT_SESSION"] == "headless:alpha"


def test_bare_headless_normalises_to_the_default_name(headless, monkeypatch):
    monkeypatch.setenv("DESKWRIGHT_SESSION", "headless")
    sess.resolve_session(as_server=True, argv=["x"])
    import os
    assert os.environ["DESKWRIGHT_SESSION"] == "headless:default"


def test_unknown_session_kind_exits_two(headless, monkeypatch):
    monkeypatch.setenv("DESKWRIGHT_SESSION", "nonsense")
...
def test_invalid_name_exits_two(headless, monkeypatch):
    monkeypatch.setenv("DESKWRIGHT_SESSION", "headless:Not A Name")
    with pytest.raises(SystemExit) as e:
        sess.resolve_session(as_server=True, argv=["x"])
    assert e.value.code == 2
...
def test_a_cold_session_is_deferred_for_the_server(headless, monkeypatch):
    ...
    sess.resolve_session(as_server=True, argv=["x"])
    import os
    assert os.environ.get("DESKWRIGHT_HEADLESS_LAZY") == "1"
...
def test_a_warm_session_is_pinned_immediately(headless, monkeypatch):
    ...
    sess.resolve_session(as_server=True, argv=["x"])
    import os
    assert "DESKWRIGHT_HEADLESS_LAZY" not in os.environ
...
def test_resolving_twice_costs_nothing(headless, monkeypatch):
    """`./mcp_server.py` resolves at import and hands off to deskwright.cli.main."""
    headless["running"].add("alpha")
    monkeypatch.setenv("DESKWRIGHT_SESSION", "headless:alpha")
    sess.resolve_session(as_server=True, argv=["x"])
    sess.resolve_session(as_server=True, argv=["x"])
...
def test_deferred_start_uses_the_name_that_was_asked_for(headless, monkeypatch):
    ...
    cold while DESKWRIGHT_SESSION still advertised it."""
    monkeypatch.setenv("DESKWRIGHT_SESSION", "headless:alpha")
...
def test_pinned_name_reads_the_environment(headless, monkeypatch):
    ...
    monkeypatch.setenv("DESKWRIGHT_SESSION", "headless:beta")
    assert sess.pinned_name() == "beta"
    monkeypatch.setenv("DESKWRIGHT_SESSION", "headless:NOPE NOPE")
...
def test_cli_self_test_starts_the_named_session_then_runs(headless, monkeypatch):
    from deskwright import cli
    ran: list[str] = []
    monkeypatch.setitem(sys.modules, "deskwright.server", _FakeServer(ran))
    monkeypatch.setenv("DESKWRIGHT_SESSION", "headless:alpha")
    rc = cli.main(["deskwright", "--self-test"])
    assert rc == 0
...
def test_cli_without_self_test_serves(headless, monkeypatch):
    ...
    ran: list[str] = []
    monkeypatch.setitem(sys.modules, "deskwright.server", _FakeServer(ran))
    rc = cli.main(["deskwright"])
    assert rc == 0
    assert ran == ["serve"]


class _FakeServer:
    """Stands in for deskwright.server so no real stdio loop or desktop is touched."""
